...
3. **Deployment.** Commit + push the edited HTML to the branch; whatever serves it
   ...
   auto-reloads into.
4. **Egnyte sync.** The dashboard is also saved for the Augustus deal team in
   Egnyte at (full path, note the different filename — no hyphens):
   `/Shared/Research/02. Private Companies/01. Portfolio Companies/Augustus/05. Market Data and Industry Analysis/Bank Charter Pipeline/bankcharterpipeline.html`.
   (An older version of this doc pointed at
   `/Shared/Private Companies/Portfolio Companies/Augustus/Bank Charter Pipeline/bank-charter-pipeline.html`
   — that path does not exist in this Egnyte tenant and returns 404/403; use the
   path above instead.) After committing and pushing, upload the refreshed
   `bank-charter-pipeline.html` to that Egnyte path, overwriting the existing
   file, so the copy the deal team opens stays current. This requires the Egnyte
   connector to be enabled for the session running the refresh — if its tools
   aren't available, note that in the run summary instead of silently skipping
   the upload.
